...
the program down to its finest stratum; what remains open, and is stated
precisely, is a canonical straightening map from arbitrary trees to
polarized ones, which we supply in the acyclic-support case. We also
...
For each class $(S,U)$, construct an explicit bijection
\[
  \{\text{spanning trees of } G\times K_2 \text{ with data } (S,U)\}
  \;\longleftrightarrow\;
  \{\text{decompositions } (T,F)\} \times \{0,1\}^{|S|-1},
\]
compatible with Proposition~\ref{prop:polar} (each tree's image should name
its \emph{polarized normal form}). Proposition~\ref{prop:rzero} solves the
acyclic-support classes; empirically the fibers over decompositions are
(usually) cosets of a subspace of the bit space $\mathbb F_2^M$, so a
straightening algorithm by cycle-by-cycle rerouting is the natural
candidate shape. By Theorem~\ref{thm:classlaw} and
Corollary~\ref{cor:telescope}, such a construction, iterated over the $n$
directions, is a bijective proof of Stanley's formula, resolving the
bijective form of \cite[Example~5.6.10]{ec2}.
\end{problem}

\begin{problem}
Relate the straightening of Problem~\ref{prob:main} to Bernardi's
spin-flip independence \cite[Thm.~1]{bernardi}: his inductive rerouting
realizes free bits on rooted forests; ours must realize $|S|-1$ free bits
per class on unrooted trees. A dictionary between the two would likely
supply the map.
\end{problem}
